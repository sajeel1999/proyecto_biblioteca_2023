...
<?php
include_once("../functions/destacados.php");
?>

<div class="container-fluid mx-auto d-flex flex-row flex-wrap mt-2 justify-content-center g--contenedor-panel">

    <?php
    for ($posLibro = 0; $posLibro < count($librosDestacados); $posLibro++) {
    ?>
        <div class="w-auto c-card text-center mt-4" data-bs-toggle="modal" data-bs-target="#<?php echo $librosDestacados[$posLibro]['id'] ?>">
            <img class="c-card__img" src="<?php echo $librosDestacados[$posLibro]['photo']; ?>" alt="sin imagen">
            <p class="c-card__title mt-3"><?php echo $librosDestacados[$posLibro]['name']; ?></p>
            <!-- <p class="c-card__text ">leer resumen</p> -->
        </div>

        <div class="modal fade g--background-color-orange-tranparent" id="<?php echo $librosDestacados[$posLibro]['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg " role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"><?php echo $librosDestacados[$posLibro]['name']; ?></h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-4">
                                <img src="<?php echo $librosDestacados[$posLibro]['photo']; ?>" alt="<?php echo $librosDestacados[$posLibro]['name']; ?>" class="w-100">
                            </div>
                            <div class="col-8">
                                <textarea disabled class="form-control g--height-30vh" autofocus><?php echo $librosDestacados[$posLibro]['resumen']; ?></textarea>
                                <p class="g--fs-20 fw-bold text-center"><?php echo $librosDestacados[$posLibro]['name_author']; ?></p>
                                <p class="g--fs-20 fw-bold">Publicada en el año: <?php echo $librosDestacados[$posLibro]['anyo_publication']; ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        ?>
                            ...
                            $habilitado = false; 
                            if ($librosDestacados[$posLibro]['disponible'] == 0) {
                                for ($posRegistro = 0; $posRegistro < count($prestaciones); $posRegistro++) {
                                    if ($prestaciones[$posRegistro]['id_libro'] == $librosDestacados[$posLibro]['id'] && $habilitado == false) {
                                        $habilitado = true;
                            ?>
                                <form action="../functions/devolver.php" method="post">
                                    <input hidden type="number" name="idLibro" value="<?php echo $librosDestacados[$posLibro]['id'] ?>">
                                    <div class="btn-group">
                                ...
                                <form action="../functions/registrarPrestacion.php" method="post">
                                    <input hidden type="number" name="registroLibro" value="<?php echo $librosDestacados[$posLibro]['id'] ?>">
                                    <div class="btn-group">
                                        <button class="btn btn-primary" type="submit">
...
<?php
include_once("../functions/nuevasPublication.php");
?>
